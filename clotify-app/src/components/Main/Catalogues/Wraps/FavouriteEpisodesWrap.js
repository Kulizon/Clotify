...
const FavouriteEpisodesWrap = () => {
  ...
  const { result, isLoading, error } = useFetch(`${hostUrl}/episodes/${likedEpisodes.join(",")},`, { method: "GET" });
  ...
  return (
    <>
      <CatalogueInfo
        ...
        image={"https://dohouse.pl/wp-content/uploads/2021/11/31.png"}
        author={username}
        amount={`${likedEpisodes.length} episodes`}
      ></CatalogueInfo>
      <PodcastCatalogue episodes={episodes} hideFollow={true} hideDescription={true}></PodcastCatalogue>
    </>
  );
};
